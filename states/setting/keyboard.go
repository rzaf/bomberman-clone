...
func updateKeyboardMapping() {
	...
	if game.IsKeyPressed("accept") {
		...
		if currentI == 0 {
			game.AddFirstKey(actions[currentJ], 0)
		} else {
			game.AddSecondryKey(actions[currentJ], 0)
		}
...
func drawKeyboardMapping() {
	...
	if currentI == 0 && currentJ == len(actions) {
	...
	if currentI == 1 && currentJ == len(actions) {
...
func GetKeyName(key int32) string {
	...
	case ray.KeyKpEqual:
		return "Num ="
	////
	case ray.KeyBackspace, ray.KeyCapsLock, ray.KeyScrollLock, ray.KeyNumLock, ray.KeyPrintScreen, ray.KeyPause, ray.KeyF1, ray.KeyF2, ray.KeyF3, ray.KeyF4, ray.KeyF5, ray.KeyF6, ray.KeyF7, ray.KeyF8, ray.KeyF9, ray.KeyF10, ray.KeyF11, ray.KeyF12:
		return "-"
	case ray.KeySpace:
		return "Space"
	case ray.KeyEscape:
